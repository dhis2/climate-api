...
```
uv run uvicorn climate_service.main:app --reload
```
...
pip install -e .
python -m uvicorn climate_service.main:app --reload
```
...
The base configuration is bundled with the package at `climate_service/data/pygeoapi/base.yml` and does not need to be copied or managed separately.
