...
- Treat `dhis2-python-client` and `dhis2eo` as strategic dependencies that can be changed to fulfill `climate-service` requirements.
- When required functionality is missing, define and implement upstream changes rather than introducing long-term local forks in `climate-service`.
...
- Preferred path: implement missing cross-project capabilities in `dhis2-python-client` and/or `dhis2eo`, then consume released versions in `climate-service`.
...
- Core map/preview and aggregation workflows required by Maps app and Climate app are available via `climate-service` without dependence on Google Earth Engine.
