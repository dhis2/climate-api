...
def test_resolve_download_dir_uses_xdg_when_no_config(monkeypatch: pytest.MonkeyPatch) -> None:
    with tempfile.TemporaryDirectory() as xdg:
        ...
        assert downloader._resolve_download_dir() == Path(xdg) / "open-climate-service" / "downloads"
...
def test_resolve_artifacts_dir_uses_xdg_when_no_config(monkeypatch: pytest.MonkeyPatch) -> None:
    with tempfile.TemporaryDirectory() as xdg:
        monkeypatch.delenv("OPEN_CLIMATE_SERVICE_CONFIG", raising=False)
        monkeypatch.setenv("XDG_DATA_HOME", xdg)
        assert ingestion_services._resolve_artifacts_dir() == Path(xdg) / "open-climate-service" / "artifacts"
