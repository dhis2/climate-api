...
from climate_service.openeo.execution import (
    _augment_with_udps,
    _bbox_to_dict,
    _RegistryOverlay,
    _temporal_to_list,
...
from climate_service.openeo.jobs import OpenEOJobService, _result_assets
from climate_service.openeo.schemas import OpenEOJobCreate, OpenEOJobRecord, OpenEOJobStatus
from climate_service.shared.time import utc_now
...
def test_augment_with_udps_returns_base_when_no_udps(monkeypatch: pytest.MonkeyPatch) -> None:
    import climate_service.openeo.udps as udp_module
...
def job_service(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> OpenEOJobService:
    monkeypatch.setattr("climate_service.openeo.jobs._JOBS_DIR", tmp_path)
...
def test_openeo_job_service_create_execute_and_get_results(
    ...
    monkeypatch.setattr(
        "climate_service.openeo.execution.run_process_graph",
