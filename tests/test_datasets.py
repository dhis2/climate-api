...
from climate_service.ingestions import services
...
from climate_service.publications.services import managed_dataset_id_for
...
def test_create_artifact_uses_streaming_plugin_for_direct_ingest(
    ...
    dataset: dict[str, object] = {
        ...
        "period_type": "daily",
        "ingestion": {
            "plugin": "climate_service.streaming.plugins.chirps3.CHIRPS3DailyPlugin",
    ...
    assert captured["plugin_path"] == "climate_service.streaming.plugins.chirps3.CHIRPS3DailyPlugin"
...
def test_create_artifact_forwards_country_code_to_streaming_plugin(
    ...
    dataset: dict[str, object] = {
        ...
        "period_type": "yearly",
        "ingestion": {
            "plugin": "climate_service.streaming.plugins.worldpop.WorldPopYearlyPlugin",
    ...
    assert captured["plugin_path"] == "climate_service.streaming.plugins.worldpop.WorldPopYearlyPlugin"
...
def test_load_streaming_plugin_rejects_non_callable_symbol() -> None:
    with pytest.raises(
        ...
        match="Failed to load ingestion.plugin 'climate_service.ingestions.services.logger'",
    ):
        services._load_streaming_plugin(
            "climate_service.ingestions.services.logger",
            params={},
        )
...
def test_load_streaming_plugin_rejects_symbol_outside_plugin_protocol(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    with pytest.raises(
        services.HTTPException,
        match="Failed to load ingestion.plugin 'climate_service.ingestions.services._NotAPlugin'",
    ):
        services._load_streaming_plugin(
            "climate_service.ingestions.services._NotAPlugin",
            params={"stage": "final"},
        )
