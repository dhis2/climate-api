...
def read_index(request: Request) -> Response:
    ...
    # Use OPEN_CLIMATE_SERVICE_BASE_URL when set so links are correct behind a reverse proxy.
    base = os.getenv("OPEN_CLIMATE_SERVICE_BASE_URL", "").rstrip("/") or str(request.base_url).rstrip("/")
