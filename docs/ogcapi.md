...
In this project pygeoapi is mounted as a sub-application at `/ogcapi`. The integration is minimal -- a single re-export in `src/climate_service/routers/ogcapi.py`:
...
Custom plugins live under `src/climate_service/routers/ogcapi/plugins/`, organized by type:
...
providers:
  - type: feature
    name: climate_service.routers.ogcapi.plugins.providers.my_provider.MyProvider
    data: /path/to/data
```
...
resources:
  my-process:
    type: process
    processor:
      name: climate_service.routers.ogcapi.plugins.processes.my_process.MyProcessor
```
