...
If you only need to run open-climate-service with built-in datasets and no custom plugins, the [setup guide](setup_guide.md) (cloning open-climate-service directly) is simpler.
...
├── open-climate-service.yaml        # instance config: extent, CRS, data_dir, plugins_dir
...
Create `open-climate-service.yaml`:
...
```bash
OPEN_CLIMATE_SERVICE_CONFIG=/absolute/path/to/my-climate-service/open-climate-service.yaml
```

And a committed `.env.example` as a template:

```bash
OPEN_CLIMATE_SERVICE_CONFIG=/path/to/my-climate-service/open-climate-service.yaml
```
