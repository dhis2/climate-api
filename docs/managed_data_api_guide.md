...
- CHIRPS3 daily uses `climate_service.providers.availability.chirps3_daily_latest_available`; this clamps sync targets to the latest complete released source month
- ERA5-Land hourly uses `climate_service.providers.availability.lagged_latest_available` with a YAML-declared `lag_hours`
- WorldPop yearly uses `climate_service.providers.availability.worldpop_release_latest_available`; this can allow configured future projection years
