...
def _validate_dataset_template(dataset: object, *, source: str) -> None:
    ...
    has_function = isinstance(function, str) and function
    has_plugin = isinstance(plugin, str) and plugin
    if not has_function and not has_plugin:
        raise ValueError(
            f"Dataset template '{dataset_id}' in {source} must define either "
            "ingestion.function (legacy download path) or ingestion.plugin (per-period Icechunk ingest)"
        )
